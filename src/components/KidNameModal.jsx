...
export default function KidNameModal({
	...
	const handleSubmit = async (e) => {
		...
		// Validate the API key live with Google Gemini API
	...
	return (
		<div className='fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md animate-in fade-in duration-300 select-none overflow-y-auto'>
			<div className='bg-gradient-to-b from-[#1C1F5E] via-[#141846] to-[#0D1030] border-4 border-amber-400/90 rounded-3xl max-w-lg w-full p-5 sm:p-7 shadow-[0_0_60px_rgba(251,191,36,0.35)] text-white relative my-auto max-h-[95vh] overflow-y-auto'>
				...
				{canDismiss && onClose && !isValidating && (
					<button
						type='button'
						onClick={() => {
							playButtonPop(soundEnabled);
				...
				<form onSubmit={handleSubmit} className='flex flex-col gap-4'>
					...
					<div className='bg-[#090B24]/80 p-3 sm:p-3.5 rounded-2xl border border-[#2C3380]'>
						...
						<input
							...
							className='w-full bg-[#0D1030] border border-[#2C3380] focus:border-pink-400 text-white text-sm sm:text-base font-bold rounded-xl px-3.5 py-2.5 placeholder:text-slate-500 focus:outline-none transition-all disabled:opacity-50'
						/>
					</div>
						...
						<div className='grid grid-cols-7 gap-1 sm:gap-1.5 mb-2'>
							{quickAges.map((age) => (
								<button
									...
									className={`py-1.5 sm:py-2 rounded-xl font-black text-xs transition-all border cursor-pointer ${
							...
							<button
								...
								className={`py-1.5 sm:py-2 rounded-xl font-black text-xs transition-all border flex items-center justify-center cursor-pointer ${
									isCustomAge ?
					...
					{/* Section 3: Google Gemini API Key (MANDATORY with Eye Password Visibility Toggle & Live Validation) */}
					<div
						className={`bg-[#090B24]/80 p-3 sm:p-3.5 rounded-2xl border-2 transition-all shadow-inner ${
							isKeyError ?
						...
						<div className='relative flex items-center'>
							<input
								...
								className={`w-full bg-[#0D1030] border text-white font-mono text-xs sm:text-sm rounded-xl pl-3.5 pr-11 py-2.5 placeholder:text-slate-500 focus:outline-none transition-all disabled:opacity-50 ${
									isKeyError ?
						...
						<span className='text-[10px] text-slate-400 mt-1 block'>
							Will be validated live with Google Gemini API upon saving.
						</span>
					</div>
						...
						{timerEnabled ?
							<div className='space-y-2 animate-in fade-in duration-200'>
								...
								<span className='text-[10px] text-slate-400 block font-semibold'>
									If time expires without selecting an answer, the solution is
									shown for 7 seconds before moving to the next question
									automatically.
								</span>
							</div>
						:	<span className='text-[10px] text-slate-400 block font-semibold'>
					...
					{/* Error Message */}
					{error && (
						<div className='bg-rose-500/20 border border-rose-500/50 rounded-2xl p-3 text-xs font-bold text-rose-200 text-center animate-shake leading-relaxed shadow-lg'>
							⚠️ {error}
						</div>
					)}
					...
					<button
						type='submit'
						disabled={isValidating}
						className={`w-full py-3.5 sm:py-4 rounded-2xl text-white font-black text-sm sm:text-base tracking-wider uppercase shadow-[0_10px_25px_rgba(245,158,11,0.4)] transition-all flex items-center justify-center gap-2 ${
							isValidating ?
								'bg-gradient-to-r from-amber-600 via-pink-600 to-purple-700 opacity-90 cursor-wait animate-pulse'
							:	'bg-gradient-to-r from-amber-400 via-pink-500 to-purple-600 hover:opacity-95 transform hover:scale-[1.01] active:scale-98 cursor-pointer'
						}`}>
						{isValidating ?
							<>
